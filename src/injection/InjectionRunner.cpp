...
bool InjectionRunner::run(const std::string& activeProfileName, const std::wstring& targetProcessName) const {
    ...
    if (config.contains("external_order") && config["external_order"].is_array()) {
        for (const auto& orderedName : config["external_order"]) {
            ...
            if (contains(diskExternals, dllName)) {
                ...
                diskExternals.erase(std::remove(diskExternals.begin(), diskExternals.end(), dllName), diskExternals.end());
            }
        }
    }
    ...
    auto getModEntrypoint = [&](const std::string& modId) -> std::filesystem::path {
    ...
    if (config.contains("mod_order") && config["mod_order"].is_array()) {
        for (const auto& orderedId : config["mod_order"]) {
            ...
            if (contains(diskMods, modId)) {
                ...
                diskMods.erase(std::remove(diskMods.begin(), diskMods.end(), modId), diskMods.end());
            }
        }
    }
    ...
    for (size_t i = 0; i < dllsToInject.size(); ++i) {
        ...
        if (i < dllsToInject.size() - 1) {
            std::this_thread::sleep_for(std::chrono::milliseconds(i == 0 ? 1000 : 2000));
        }
    }
