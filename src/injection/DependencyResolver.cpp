...
// --- Version Helpers Implementation (AI provided) ---
...
bool Version::satisfies(const std::string& constraint) const {
    ...
    if (op == ">=") {
        ...
        // Compatible version (Same major, installed minor/patch >= required minor/patch)
        if (major != reqVer.major) return false;
        return *this >= reqVer;
    } else {
...
bool DependencyResolver::resolve(const std::string& activeProfileName, std::vector<std::string>& outErrors) const {
    // Get path to "mods" directory: profiles/[Active_Profile]/mods
    std::filesystem::path modsPath = m_profileManager.getProfilePath(activeProfileName) / "mods";
    ...
    //    - For each mod, read the "dependencies" object (if present) from manifest.json:
    //      "dependencies": {
    //         "com.mod.A": ">=1.0.0",
    //         "com.mod.B": "^2.0.0"
    //      }
    ...
    //    - For each dependency target (e.g., depId = "com.mod.A", constraint = ">=1.0.0"):
    //      a. Check if depId exists in installedMods. If not, add error to outErrors:
    //         "Mod [modId] requires [depId] but it is not installed."
    //      b. If installed, parse its version: Version installedVer = Version::parse(installedMods[depId]);
    //      c. Check if installedVer.satisfies(constraint). If not, add error to outErrors:
    //         "Mod [modId] requires [depId] ([constraint]) but version [installedMods[depId]] is installed."
        if (manifest.contains("dependencies")) {
            for (const auto& [depId, constraintJson] : manifest["dependencies"].items()) {
                ...
                if (installedMods.find(depId) == installedMods.end()) {
                    outErrors.push_back("Mod " + modId + " requires " + depId + " but it is not installed.");
                } else {
                    Version installedVer = Version::parse(installedMods[depId]);
                    if (!installedVer.satisfies(constraint)) {
                        outErrors.push_back("Mod " + modId + " requires " + depId + " (" + constraint + ") but version " + installedMods[depId] + " is installed.");
                    }
                }
            }
        }
