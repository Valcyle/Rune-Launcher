...
class AppWindow {
public:
  AppWindow(ProfileManager pm, ModImporter importer,
            DependencyResolver resolver, InjectionRunner runner);
  ~AppWindow();

  // Launcher Version Definition
  static inline const std::string RUNE_LAUNCHER_VERSION = "1.0.0-beta.8";
...
private:
  ...
  void postEventToUI(const std::string &eventName,
                     const std::string &jsonDetail) const;
